...
func _ready() -> void:
	if direction == -1:
		...
		while inst_pos.x > left_corner:
			...
			inst_pos.x -= frequency_seconds / Engine.get_frames_per_second() * speed
			print(str(frequency_seconds / Engine.get_frames_per_second() * speed))
	else:
...
func instantiate(instance_position):
